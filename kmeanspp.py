...
class KMeans:
    ...
    def get_params(self):
        return {"n_clusters": self.n_clusters, "seeding_algorithm": self.seeds_algo,
                "max_iterations": self.max_iterations, "t": self.t, "l":self.l}
    ...
    def fit(self, X):
        ...
        while itteration < self.max_iterations and cluster_updated:
            if DEBUG:
                print('iteration ' + str(itteration))
            
            centroids_to_x_dict = self.group_x_by_clusters(X)
            
            if DEBUG:
                print('closest centroid to sample list')
                print(centroids_to_x_dict.values())
            
            old_centroids = self.centroids
            
            self.centroids = [np.mean(np.array([X[x_idx] for x_idx in centroids_to_x_dict[c_idx]]), axis=0) for c_idx in
            ...
            cluster_updated = any(
                not np.array_equal(self.centroids[c_idx], old_centroids[c_idx]) for c_idx in range(len(self.centroids)))
            # print(self.centroids)
    ...
    def overseeding_scalable_kmeanspp(self, X):
        ...
        if DEBUG:
            print("entering overseeding")
            print('random first index in B')
            print(c_indices)
        for i in range(self.t):
            ...
            if DEBUG:
                print('Samples indices joined to B:')
                print(c_indices)
        return c_indices
    ...
    def seeding_scalable_kmeanspp(self, B, X):
        ...
        W = []
        for b in B:
            points_closest_to_c = [x_idx for x_idx in range(len(X)) if is_c_closet_to_x(x=X[x_idx], c=b,
                                                                                        all_clusters=B) and x_idx not in calculated_x_indices]
